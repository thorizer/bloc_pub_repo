...
class Nominatim {
  ...
  static Future<List<Place>> searchByName({
    ...
    return data
        .map<Place>((dynamic p) => Place.fromJson(p as Map<String, dynamic>))
        .toList();
  }
  ...
  static Future<Place> reverseSearch({
    ...
    try {
      ...
      final nominatimResponse = await http.post(
        urit,
        headers: {
      ...
      statusCode = nominatimResponse.statusCode;
      if (nominatimResponse.body.startsWith('{')) {
        data = json.decode(nominatimResponse.body) as Map<String, dynamic>;
      } else {
        placeData = Place(
          displayName: '',
          boundingBox: [],
      ...
      placeData = (Place.fromJson(data));
    } catch (e) {
      //print('nominatim exp: $e');
      /* _httpClient.close();
      ...
      //print(placeData);
      //print('NominatimStatusCode: ' + statusCode.toString());
      /* _httpClient.close();
      _httpClient = http.Client(); */
    }
  ...
  static Future<bool> status() async {
    ...
    try {
      final response = await http.get(urit);
